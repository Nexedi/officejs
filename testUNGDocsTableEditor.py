...
class TestUNGDocsTableEditor(UNGTestMixin):
    def test_fill_some_cells_in_web_table(self):
        ...
        self.selenium.open("ERP5Site_createNewWebDocument?"
        ...
        self.assertEqual("Spreadsheet Playground", self.selenium.get_text(
                                                 "//td[@id=\"jSheetTitle_0\"]"))
        self.assertEqual("MENU", self.selenium.get_text("//td"
                           "[@id=\"jSheetMenu_0\"]/span[@class=\"rootVoice\"]"))
        self.selenium.run_script("$(\"#0_table0_cell_c0_r0\")"
                                                           ".html(\"Gabriel\")")
        self.selenium.run_script("$(\"#0_table0_cell_c1_r1\")"
                                                          ".html(\"Monnerat\")")
        self.selenium.click("//button[@class=\"save\"]")
        for page_reload in range(2):
            self.selenium.wait_for_page_to_load("30000")
        self.assertEqual("Gabriel", self.selenium.get_text(
                                             "//td[@id='0_table0_cell_c0_r0']"))
        self.assertEqual("Monnerat", self.selenium.get_text(
                                             "//td[@id='0_table0_cell_c1_r1']"))
